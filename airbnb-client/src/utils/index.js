...
export const getBill = (price, guest, night, discount) => {
  let preGuest = price * guest;
  let preNights = preGuest * night;
  let preFeeC = preNights * 0.05;
  let preFeeA = preNights * 0.05;
  let preDiscount = preNights * discount;
  return {
    total: Math.floor(preNights - preDiscount + preFeeC + preFeeA),
    base: Math.floor(preNights),
    cleanFee: Math.floor(preFeeC),
    airbnbFee: Math.floor(preFeeA),
    discount: Math.floor(preDiscount)
  };
};
